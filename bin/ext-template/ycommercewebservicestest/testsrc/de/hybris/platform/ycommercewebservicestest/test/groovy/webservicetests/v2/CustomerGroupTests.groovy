...
class CustomerGroupTests extends BaseWSTest {
	...
	public void testCreateCustomerGroup() {
		...
		String postBody = "groupId=${randomUID}&localizedName=bbbbbb"
	...
	public void testCreateCustomerGroupWithoutName() {
		access_token = testUtil.getAccessToken("customermanager", "1234")
		String postBody = "groupId=${randomUID}_withoutName"
		testUtil.getSecureConnection("/customergroups", "POST", "XML", HttpURLConnection.HTTP_CREATED, postBody, null, access_token );
	}
	...
	public void testCreateCustomerGroupAndSpecifyLang() {
		access_token = testUtil.getAccessToken("customermanager", "1234")
		String postBody = "groupId=${randomUID}_withLangDe&localizedName=bbbbbb&lang=de"
		testUtil.getSecureConnection("/customergroups", "POST", "XML", HttpURLConnection.HTTP_CREATED, postBody, null, access_token );
	}
	...
	public void testCreateCustomerGroupWithDuplicateId() {
		access_token = testUtil.getAccessToken("customermanager", "1234")
		String postBody = "groupId=${randomUID}_duplicated"
		...
		assert response.errors[0].type == 'ModelSavingError'
	}
	...
	public void testTryToCreateCustomerGroupAsNotAdminUser() {
		UsersTest usersTests = new UsersTest();
		def uid = usersTests.registerUser();
		access_token = testUtil.getAccessToken(uid, UsersTest.password);
		String postBody = "groupId=${randomUID}&localizedName=bbbbbb"
	...
	public void testAssignUserToCustomerGroup() {
		...
		def con = testUtil.getSecureConnection("/users", 'POST', 'XML', HttpURLConnection.HTTP_CREATED, body, null, client_credentials_token)
		// add second user
		body = "login=${userUid2}&password=password&firstName=firstName&lastName=lastName&titleCode=dr"
		con = testUtil.getSecureConnection("/users", 'POST', 'XML', HttpURLConnection.HTTP_CREATED, body, null, client_credentials_token)

		// add customer group
		body = "groupId=${customerGroup1}"
		...
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members", "PATCH", "XML", HttpURLConnection.HTTP_OK, putBody, null,access_token );

		putBody = "members=${userUid2}"
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members", "PATCH", "JSON", HttpURLConnection.HTTP_OK, putBody, null,access_token );

		// add one more time user to the same group - no error expected
		putBody = "members=${userUid1}"
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members", "PATCH", "XML", HttpURLConnection.HTTP_OK, putBody, null,access_token );

		// add 0 memebers to group - error expected
		putBody = ""
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members", "PATCH", "XML", HttpURLConnection.HTTP_BAD_REQUEST, putBody, null,access_token );

		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}", "GET", "XML", HttpURLConnection.HTTP_OK, null, null,access_token );
	...
	public void testRemoveUserFromCustomerGroup() {
		...
		// add member
		def putBody = "members=${userUid1}&members=${userUid2}"
		...
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members/${userUid2}" , "DELETE", "JSON", HttpURLConnection.HTTP_OK, null, null,access_token );
		// remove one more time user from the same group - no error expected
		con = testUtil.getSecureConnection("/customergroups/${customerGroup1}/members/${userUid1}" , "DELETE", "XML", HttpURLConnection.HTTP_OK, null, null,access_token );
		//check if ROLE_ADMINGROUP is required
		UsersTest usersTests = new UsersTest();
		def uid = usersTests.registerUser();
		def simpleUserAccessToken = testUtil.getAccessToken(uid, UsersTest.password)
	...
	public void testGetAllCustomerGroupsXML() {
		access_token = testUtil.getAccessToken("customermanager", "1234")
		def con = testUtil.getSecureConnection("/customergroups?pageSize=2147483647&fields=DEFAULT,userGroups(FULL)","GET","XML", HttpURLConnection.HTTP_OK, null, null, access_token)
		...
		def body = "groupId=${uid}"
		con = testUtil.getSecureConnection("/customergroups", "POST","XML",HttpURLConnection.HTTP_CREATED,body, null, access_token)
		con = testUtil.getSecureConnection("/customergroups?pageSize=2147483647","GET","XML", HttpURLConnection.HTTP_OK, null, null, access_token)
		...
		for(int currPage = 0; currPage < maxPages ; currPage++) {
			...
			response = testUtil.verifiedXMLSlurper(con,true);
			assert response.userGroups.size() <= pageSize
			assert response.pageSize != null
	...
	public void testGetAllCustomerGroupsJSON() {
		access_token = testUtil.getAccessToken("customermanager", "1234")
		def con = testUtil.getSecureConnection("/customergroups?pageSize=2147483647&fields=DEFAULT,userGroups(FULL)","GET","JSON", HttpURLConnection.HTTP_OK, null, null, access_token)
		...
		def body = "groupId=${uid}"
		con = testUtil.getSecureConnection("/customergroups", "POST","XML",HttpURLConnection.HTTP_CREATED,body, null, access_token)
		con = testUtil.getSecureConnection("/customergroups?pageSize=2147483647","GET","JSON", HttpURLConnection.HTTP_OK, null, null, access_token)
