			...
			<div class="content">
				...
				<div>
		            <c:forEach items="${paymentTypes}" var="paymentType">
		            	<spring:theme code="sap.paymenttype.${paymentType.code}" var="displayName"/>
		                <form:radiobutton checked="true" disabled="true" path="paymentTypes" id="PaymentTypeSelection_${paymentType.code}" name="PaymentType" value="${paymentType.code}" label="${displayName}"/> <br>
		            </c:forEach>
		            <br>
					...
					<input type="text" id="PurchaseOrderNumber"  name="PurchaseOrderNumber" value="${cartData.purchaseOrderNumber}" maxlength="255" />
				</div>
			</div>
		</div>
	</ycommerce:testId>
