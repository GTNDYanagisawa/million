...
<template:page pageTitle="${pageTitle}">
	...
	<c:if test="${not empty cartData.entries}">
		<spring:url value="${continueUrl}" var="continueShoppingUrl" htmlEscape="true"/>
			...
			<c:if test="${cartData.backendDown}">
				<button  class="checkoutButton positive right" type="button" disabled="disabled"  
				data-checkout-url="${checkoutUrl}"><spring:theme code="checkout.checkout" /></button>
			</c:if>
			<c:if test="${not cartData.backendDown}">
				<button  class="checkoutButton positive right" type="button"   
				data-checkout-url="${checkoutUrl}"><spring:theme code="checkout.checkout" /></button>
			</c:if>
			...
			<c:if test="${cartData.backendDown}">
				<button class="checkoutButton positive right" type="button" disabled="disabled" data-checkout-url="${checkoutUrl}"><spring:theme code="checkout.checkout" /></button>
			</c:if>				
			<c:if test="${not cartData.backendDown}">
