	...
	private class ScheduleTaskWhileTriggerEventService extends DefaultTaskService
	{
		...
		ScheduleTaskWhileTriggerEventService(final ModelService ms, final FlexibleSearchService fs)
		{
			...
			setScheduleAndTriggerStrategy(strategy = new DefaultScheduleAndTriggerStrategy()
			{
				...
				@Override
				protected void retryMatchConditionForEvent(final String uniqueId)
				{
					super.retryMatchConditionForEvent(uniqueId);
					retryCalled = true;
				}

				@Override
				protected boolean insertEventConditionOrRetryMatch(final String uniqueId, final Date expirationDate)
				{
					scheduleTaskFromOtherThread(uniqueId);

					return super.insertEventConditionOrRetryMatch(uniqueId, expirationDate);
				}
