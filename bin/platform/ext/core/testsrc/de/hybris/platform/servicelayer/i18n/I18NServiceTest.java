...
public class I18NServiceTest extends ServicelayerTransactionalBaseTest
{
	...
	/**
	 * PLA-10438 Nullpointer in DefaultLocalizationService
	 */
	@Test
	public void sessionTimeoutTest() throws InterruptedException
	{
		final int timeout = 2000;
		// PLA-10438 Nullpointer in DefaultLocalizationService - check 1: no session created at all
		Locale loc = i18nService.getCurrentLocale();

		JaloSession session = JaloSession.getCurrentSession();
		session.setTimeout(timeout / 1000);

		for (int i = 0; i < 10; ++i) // bypass problem with session timestamp update (see SessionTest.sessionCreationTest())
		{
			session = JaloSession.getCurrentSession();
			Assert.assertNotNull(session);
		}

		Assert.assertEquals("Session timeout changed in meantime", timeout / 1000, session.getTimeout());

		Assert.assertFalse("Session must NOT be closed here", session.isClosed());
		Assert.assertFalse("Session must NOT be expired here", session.isExpired());

		Thread.sleep(timeout + 100); // sleep little bit above the timeout

		Assert.assertTrue("session here is expected to be expired", session.isExpired());

		// PLA-10438 Nullpointer in DefaultLocalizationService  - check 2: session expired
		// if fails we expect to throw exception
		loc = i18nService.getCurrentLocale();
		Assert.assertNotNull(loc);
	}

	@Test
	public void testPLA11796()
	{
		final long overIntegerMaxTimeout = Integer.MAX_VALUE + 10;
		final int a30daysTimeout = 2592000;
		final int smallTimeout = 900;

		final JaloSession session = JaloSession.getCurrentSession();
		final int oldTimeOut = session.getTimeout();

		try
		{
			Assert.assertTrue("original timeout was not positive value!!", session.getTimeout() > 0);

			session.setTimeout(smallTimeout);
			Assert.assertTrue(session.getTimeout() == smallTimeout);
			Assert.assertFalse(session.isExpired());

			session.setTimeout(a30daysTimeout);
			Assert.assertTrue(session.getTimeout() == a30daysTimeout);
			Assert.assertFalse(session.isExpired());

			//setting invalid state
			session.setTimeout((int) overIntegerMaxTimeout);
			Assert.assertTrue(session.getTimeout() < 0);
			Assert.assertFalse(session.isExpired());
