...
public class SessionTest extends HybrisJUnit4Test
{
	...
	public void testCleanExpiredSessionWithDeletedUser() throws ConsistencyCheckException, JaloSecurityException
	{
		...
		jSession.setTimeout(TTL); // 5 seconds until it gets stale
		...
		// wait until session becomes expired
		final long maxWait = System.currentTimeMillis() + ((TTL * 2) * 1000);
		while (!jSession.isClosed() && System.currentTimeMillis() < maxWait)
		{
			jaloConnection.cleanExpiredSessions(true);
		}
	...
	protected void compareSessions(final JaloSession jaloSession, final JaloSession copy)
	{
		...
		assertEquals(jaloSession.getTenant(), copy.getTenant());
		assertNull(copy.getHttpSession());
	}
