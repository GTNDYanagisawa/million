...
public class PermissionCheckingServiceTest extends AbstractPermissionServiceTest
{
	...
	{
		Assert.assertTrue(permissionCheckingService.checkGlobalPermission(userService.getAdminUser(), NOT_IMPORTANT_PERMISSION_NAME)
				  .isGranted());
	}
	...
	{
		Assert.assertTrue(permissionCheckingService.checkAttributeDescriptorPermission(
				  ItemModel._TYPECODE, "pk", userService.getAdminUser(), NOT_IMPORTANT_PERMISSION_NAME).isGranted());
	}
	...
	public void testCheckItemPermissionForAdmin()
	{
		...
		Assert.assertTrue(
				  permissionCheckingService.checkItemPermission(newCountry, userService.getAdminUser(), NOT_IMPORTANT_PERMISSION_NAME)
							 .isGranted());
	}
	...
	public void testCheckGlobalPermissionDirectlyAssigned()
	{
		...
		final PrincipalModel[] principals =
				  {testUser1, testUser2};
		final PermissionCheckValue[] expectedCheckResults =
				  {ALLOWED, DENIED, NOT_DEFINED, ALLOWED};
