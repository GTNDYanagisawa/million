...
public class ContextMapUnitTest
{
	...
	public void testNestedMaps()
	{
		...
		org.junit.Assert.assertEquals(Arrays.asList("0", "0", "0"), new ArrayList(root.values()));
		assertSetsEqual(root.keySet(), "foo-root", "bar-root", "roo-root");


		org.junit.Assert.assertEquals(Arrays.asList("1", "0"), new ArrayList(firstLevel.values()));
		assertSetsEqual(firstLevel.keySet(), "foo-root", "bar-root", "roo-root", "foo-1");

		org.junit.Assert.assertEquals(Arrays.asList("2", "1", "0"), new ArrayList(secondLevel.values()));
		assertSetsEqual(secondLevel.keySet(), "foo-root", "roo-root", "foo-1", "foo-2");


		org.junit.Assert.assertEquals(Arrays.asList("3", "1", "0"), new ArrayList(thirdLevel.values()));
		assertSetsEqual(thirdLevel.keySet(), "roo-root", "foo-3", "foo-1");
