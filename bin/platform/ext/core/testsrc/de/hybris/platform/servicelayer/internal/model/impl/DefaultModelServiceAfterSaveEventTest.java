...
public class DefaultModelServiceAfterSaveEventTest extends ServicelayerBaseTest
{
	...
	private volatile PK pk;
	private volatile String newCode;
	...
	private volatile String codeFromListener;
	...
	public void shouldReturnRefreshedModelInAfterSaveEvent() throws InterruptedException
	{
		pk = order.getPk();
		newCode = "TEST 01";
		OrderModel orderModel = modelService.get(pk);
		...
		assertEquals("timed out while waiting for event", true, latch.await(TIMEOUT, TimeUnit.SECONDS));

		assertEquals("code from AfterSaveEvent listener is outdated", newCode, codeFromListener);
	...
	private AfterSaveListener createEventListener()
	{
		return new AfterSaveListener()
		{
			...
			public void afterSave(final Collection<AfterSaveEvent> events)
			{
				for (final AfterSaveEvent ase : events)
				{
					...
					if (pk.equals(DefaultModelServiceAfterSaveEventTest.this.pk))
					{
						final OrderModel orderModel = modelService.get(pk);
						codeFromListener = orderModel.getCode();
						LOG.info("CODE FROM AFTER SAVE EVENT: " + codeFromListener);
						latch.countDown();
					}
				}
			}
