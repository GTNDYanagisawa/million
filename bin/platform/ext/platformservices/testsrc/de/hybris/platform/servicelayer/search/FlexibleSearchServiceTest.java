...
public class FlexibleSearchServiceTest extends ServicelayerTransactionalTest
{
	...
	{
		final String query =
				  "select distinct({passwordanswer}) from {customer}"; //this query (should) returns a single row with NULL
	...
	public void testTranslate()
	{
		final String expectedTranslatedSqlQuery = "SELECT  item_t0.PK  FROM " + (getTablePrefix()) +
				  "languages item_t0 WHERE ( item_t0.ActiveFlag  = ?) AND (item_t0.TypePkString=? ) order by  item_t0.PK ";
		...
		Assertions.assertThat(translationResult.getSQLQuery())
				  .isEqualToIgnoringCase(expectedTranslatedSqlQuery);//intentional case insensitive
	...
	public void testTranslateWithCustomSession()
	{
		...
		{
			final List<CatalogVersionModel> cvModelList =
					  searchQuery.getCatalogVersions() == null ? new ArrayList<CatalogVersionModel>()
