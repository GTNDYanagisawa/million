...
public class DefaultCatalogVersionDao implements CatalogVersionDao
{
			...
			final String relation)
	{
		...
		searchQuery.append("SELECT DISTINCT {rel:").append(Link.TARGET).append("} ").append("FROM {").append(relation).append(
				"* AS rel }").append(" WHERE {rel:").append(Link.SOURCE).append("} in (?principals)");
